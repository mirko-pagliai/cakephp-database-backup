...
* `BackupTrait::getDriver()` method has become `AbstractBackupUtility::getDriver()` and it no longer accepts a connection
  as an argument, but directly uses the one set by the configuration;
