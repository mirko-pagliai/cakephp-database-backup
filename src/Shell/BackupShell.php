...
class BackupShell extends Shell
{
    ...
    public function deleteAll()
    {
        ...
        foreach ($deleted as $file) {
            $this->verbose(__d('database_backup', 'Backup `{0}` has been deleted', Filesystem::instance()->rtr($file)));
        }
    ...
    public function export()
    {
        try {
            ...
            if ($this->param('filename')) {
                $instance->filename((string)$this->param('filename'));
                ...
                $instance->compression((string)$this->param('compression'));
            }

            //Exports
            $file = $instance->export();

            $this->success(__d('database_backup', 'Backup `{0}` has been exported', (new Filesystem())->rtr($file)));

            //Sends via email
            if ($this->param('send')) {
                $this->send($file, (string)$this->param('send'));
            }

            //Rotates
            if ($this->param('rotate')) {
                $this->rotate((int)$this->param('rotate'));
            }
        } catch (Exception $e) {
    ...
    public function import($filename)
    {
        try {
            ...
            $this->success(__d('database_backup', 'Backup `{0}` has been imported', (new Filesystem())->rtr($file)));
        } catch (Exception $e) {
            $this->abort($e->getMessage());
        }
    }
    ...
    public function send($filename, $recipient)
    {
        try {
            ...
            $this->success(__d('database_backup', 'Backup `{0}` was sent via mail', (new Filesystem())->rtr($filename)));
        } catch (Exception $e) {
            $this->abort($e->getMessage());
        }
    }
